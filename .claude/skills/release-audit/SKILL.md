---
name: release-audit
description: Run the full release audit of the DeFlow public documentation site — information architecture and onboarding per audience, visuals and layout, every claim and the product-facts snapshot against the Core ledger, content quality, accessibility, SEO, brand consistency, code, Vercel delivery and security — against the single audit pack in audit/. Optional argument names the gate (private-testnet, public-testnet, mainnet). Use when the founder asks for a full audit or a pre-launch review of this repository; not for reviewing one change or one issue.
---

# Release audit — the DeFlow public documentation site (docs.deflowlabs.io)
...
1. There is one audit pack, `audit/` at the root of this repository, updated in place and never published: no Content collection, route, search index or sitemap may include it. A new run reconciles every item of the previous run (charter section 5) before writing new findings; finding IDs keep the `DOC-` prefix and their numbers.
2. GitHub Issues in this repository are its only backlog, and `docs/PRODUCT_FACTS.yaml` in `deflowlabs/core` is the only source of product truth, read through `gh api`; `content/data/product-facts.yaml` is a synced snapshot the audit never edits. The pack proposes; it does not own status.
3. The charter holds no snapshot facts. Every count, page, workflow and variable is derived at run time; where the repository disagrees with the charter, the repository wins — record the difference in `audit/baseline.json` and continue.
