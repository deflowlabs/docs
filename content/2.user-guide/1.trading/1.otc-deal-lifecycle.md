...
stateDiagram-v2
  [*] --> Draft
  Draft --> Invited: Send terms
  Invited --> Accepted: Counterparty accepts
  Invited --> Cancelled: Invitation cancelled or expires
  Accepted --> Deployed: Escrow created on Sepolia
  Deployed --> PartiallyFunded: One party funds
  PartiallyFunded --> Funded: Both parties fund
  Deployed --> Refunded: Deadline and abort rules
  PartiallyFunded --> Refunded: Deadline and abort rules
  Funded --> Settled: Settlement
  Funded --> Disputed: Counterparty opens dispute
  Disputed --> Refunded: Governance resolution
  Disputed --> Settled: Governance resolution
```
