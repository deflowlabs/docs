...
DeFlow partners with Zyphe, a certified identity verification provider, for all KYC and AML checks. The verification process is designed so that DeFlow never handles personal documents:

1. The user is redirected to Zyphe's secure verification portal
2. Zyphe conducts the identity check using the user's government-issued ID and biometric data
3. Zyphe returns only a **status flag** to DeFlow (Approved, Pending, Blocked, or Rejected)
4. No personal data, document images, or biometric information is transmitted to DeFlow

| Data | Stored by DeFlow | Stored by Zyphe |
|------|:----------------:|:---------------:|
...
Wallet addresses are screened against international sanctions databases (OFAC, EU, UN, OFSI, and others) via the Scorechain integration. The screening returns a pass or fail result. Wallet addresses themselves are not considered PII, as they are public blockchain data.

### Error Reporting and Observability

DeFlow uses Sentry for error monitoring. All error reports are processed through a custom filter that strips:
...
To understand how the smart contracts that handle your funds are designed and secured, see [Smart Contract Security](/security/smart-contract-security).
